          ...
          <div class="col-lg-12 col-md-12 col-sm-12">
            <div class="card">
              <div class="card-header d-flex justify-content-between align-items-center">
                <h3 class="card-title">Edit Quiz - {{$quiz->name}}</h3>
                <a href="{{ route('quiz.view') }}" class="btn btn-success btn-sm"><i class="fa fa-list"></i> Quiz List</a>
              </div>

              <!-- /.card-header -->
                 <form method="post" action="{{ route('quiz.update', $quiz->id) }}" id="quickForm" novalidate="novalidate"> 
                  @csrf
                    <div class="card-body">
                      ...
                      <div class="row">
                        <div class="col-md-6">
                          <div class="form-group">
                            ...
                            <input type="text" value="{{$quiz->name}}" name="quiz" class="form-control" id="quiz" placeholder="Enter Quiz Name">
                          </div>
                        </div>
                        <div class="col-md-2">
                          <div class="form-group">
                            <label for="total_mark">Total Mark</label>
                            <input type="number" value="{{$quiz->total_mark}}" name="total_mark" id="total_mark" class="form-control" placeholder="Enter Total Mark">
                          </div>
                        </div>
                        <div class="col-md-2">
                          <div class="form-group">
                            <label for="time">Time (Minute Count)</label>
                            <input type="number" value="{{$quiz->time}}" name="time" id="time" class="form-control" placeholder="Ex: 10">
                          </div>
                        </div>
                        <div class="col-md-2">
                          <div class="form-group">
                            <label for="total_question">Total Question</label>
                            <input type="number" value="{{$quiz->total_question}}" name="total_question" id="total_question" class="form-control" placeholder="Total Question">
                          </div>
                        </div>
                      </div>
                    </div>
